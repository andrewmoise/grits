export const tests = [
  {
    ...
    async fn(shell, scratch) {
      ...
      await shell.eval(`gsh.p('${scratch}/hello.txt').rm()`);
      let threw = false;
      try {
  ...
  },
  {
    ...
    async fn(shell, scratch) {
      await shell.eval(`gsh.p('${scratch}/subdir').mkdir()`);
      let threw = false;
      try {
        await shell.eval(`gsh.p('${scratch}/subdir').rm()`);
      } catch (e) {
  ...
  },
  {
    ...
    async fn(shell, scratch) {
      await shell.eval(`gsh.p('${scratch}/subdir').mkdir()`);
      await shell.eval(`gsh.p('${scratch}/subdir').rm({r:1})`);
      let threw = false;
      try {
  ...
  },
  {
    ...
    async fn(shell, scratch) {
      ...
      try {
        ...
        await shell.eval(`gsh.p('${scratch}/f').read().rm()`);
      } catch (e) {
  ...
  },
  {
    ...
    async fn(shell, scratch) {
      let threw = false;
      try {
        await shell.eval(`gsh.p('${scratch}/nope.txt').rm()`);
      } catch (e) {
        if (e.message.includes('not found')) threw = true;
        else throw e;
      }
  ...
  },
  {
    ...
    async fn(shell, scratch) {
      await shell.eval(`gsh.p('${scratch}/nope.txt').rm({f:1})`);
    },
  },
  {
    label: 'rm with {f:1} still fails on directory',
    async fn(shell, scratch) {
      await shell.eval(`gsh.p('${scratch}/dir').mkdir()`);
      let threw = false;
      try {
        await shell.eval(`gsh.p('${scratch}/dir').rm({f:1})`);
      } catch (e) {
        if (e.message.includes('is a directory')) threw = true;
        else throw e;
      }
